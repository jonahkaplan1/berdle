<script lang="ts">
	...
	function submitWord() {
		if (game.board.words[game.guesses].length !== COLS) {
			...
			if (modeData.modes[$mode].name === 'BECKY MODE') {
				beckyScore += calcPoints(word, game.board.words[game.guesses]);
			}
...
<main class:guesses={game.guesses !== 0} style="--rows: {ROWS}; --cols: {COLS}">
	...
	<Board
		...
		svgIconPath={modeData.modes[$mode].svgIconPath}
	/>
	<Keyboard
		on:keystroke={() => {
...
<Modal bind:visible={showStats}>
	...
	{:else}
		...
		{/if}
		{#if modeData.modes[$mode].name === 'BECKY MODE'}
			<BeckyStatistics data={stats} />
			<BeckyDistribution distribution={stats.scoreGroups} {game} />
		{/if}
